...
function createMenu(params)
	...
	menu.mode = none
	menu.param = params.param
	menu.emboss = params.emboss
		...
		function button:touch(event)
			...
			elseif event.phase == "ended" then
				...
				if self:inside(event) then				
					self.listener(self.listenerParam)
	...
	for i = 1, #menu.items do
		local item = menu:button(menu.items[i], width, itemHeight, textHeight, insetX, insetY, menu.emboss)
		item.x = xPos
		item.y = yPos
		yPos = yPos + spacing
		xPos = xPos + xSpacing
